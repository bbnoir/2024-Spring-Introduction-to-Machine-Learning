...
def main():
    ...
    if args.part == "1":
        # Part 1: Different number of neurons with 1 hidden layer
        print("Part 1: Different number of neurons with 1 hidden layer")
        ...
        for hidden_dim in hidden_dim_list:
            ...
            history = train(model, trainloader, testloader, criterion, optimizer, n_epochs, device)
            # save history
            history_path = f"./history/hiddendim_{hidden_dim}_layer_1.pkl"
            torch.save(history, history_path)
            print(f"Saving history to {history_path}")
    elif args.part == "2":
        # Part 2: Different number of training data with 2 hidden layers of 100 neurons
        print("Part 2: Different number of training data with 2 hidden layers of 100 neurons")
        ...
        for train_data_size in train_data_size_list:
            ...
            optimizer = torch.optim.Adam(model.parameters())
            history = train(model, cropped_trainloader, testloader, criterion, optimizer, n_epochs, device)
            # save history
            history_path = f"./history/train_data_size_{train_data_size}.pkl"
            torch.save(history, history_path)
            print(f"Saving history to {history_path}")
    elif args.part == "3":
        # Part 3: Different number of hidden layers with 100 neurons
        print("Part 3: Different number of hidden layers with 100 neurons")
